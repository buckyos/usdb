...
此目录提供独立的 `usdb-public` 工具和发布通道。目标机器不需要安装 `usdb-node`、Bitcoin、
...
网络身份来自 `networks/usdb-testnet-v0.json`，发布时与 USDB canonical network bundle 对照；
工具的版本是 `usdb-public-vX.Y.Z`，与节点 `usdb-testnet-v0-rN` 分开升级，网络 chain ID 不变。
...
使用运行 public-services 的普通运维用户执行，默认无需 sudo。例如版本 `0.1.0`：

```bash
bash <(curl -fsSL https://github.com/buckyos/usdb/releases/download/usdb-public-v0.1.0/install-usdb-public-v0.1.0.sh)
```
...
| 安装内容 | 默认位置 |
| --- | --- |
| 各版本独立目录 | `~/.local/share/usdb-public/releases/usdb-public-vX.Y.Z` |
| 当前版本指针 | `~/.local/share/usdb-public/current` |
| 命令入口 | `~/.local/bin/usdb-public` |
...
或者直接调用 `~/.local/bin/usdb-public`。安装器不会自动改写 shell 启动文件。

安装脚本支持 `--install-root`、`--bin-dir` 和 `--config-file`；默认路径满足普通单用户部署。
例如在一键安装命令之后追加 `--install-root /data/usdb-public` 可调整版本存储位置。
已有 `usdb-public` 命令若不属于该安装目录，会报错并保留它，不会覆盖其他工具或手工安装。

源码开发时可直接运行 `public-services/usdb-public`，此时仅网关在目标 Docker 中从源码构建；
...
```bash
usdb-public prepare
usdb-public preflight
usdb-public up
usdb-public status
usdb-public logs --follow
```
...
bash <(curl -fsSL https://github.com/buckyos/usdb/releases/download/usdb-public-v0.2.0/install-usdb-public-v0.2.0.sh)
usdb-public prepare --replace
usdb-public up
usdb-public check
```
...
该选项不识别 public-services，也不控制它；管理员负责在后续调整时继续保持整机预算一致。
...
```bash
python3 public-services/package_release.py --check-network
python3 public-services/package_release.py --version 0.1.0 \
  --gateway-image 'ghcr.io/buckyos/usdb-public-gateway@sha256:<真实构建digest>' \
  --output-dir /secure/public-release
```

源码开发包会记录 `source_dirty`，发布流水线应来自已提交代码。网络 catalog 从 canonical bundle
生成，变化需 review；不以远端 RPC 返回的 chain ID/genesis 自动覆盖期望身份。
本包按明确文件列表构建，不打包 node.env、snapshot、SourceDAO 私有 state 或本机数据库凭据。

参考：[Blockscout RPC 要求](https://docs.blockscout.com/setup/requirements/node-tracing-json-rpc-requirements)、
