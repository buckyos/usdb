...
from install_release import RELEASE_ID, extract_release, sha256, verify_release
from package_release import FILES
from release_installer import render_installer

REPOSITORY = "buckyos/usdb"
BUILD_WORKFLOW = ".github/workflows/usdb-public-release.yml"
...
def release_assets(release, release_id):
    ...
    archive, installer = release_id + ".tar.gz", "install-" + release_id + ".sh"
...
def verify_payload(directory, release_id, revision, read_source):
    ...
    with tempfile.TemporaryDirectory(prefix="usdb-public-verify-") as temporary:
        ...
        if (gateway.get("tag") != release_id or not re.fullmatch(
                r"ghcr\.io/buckyos/usdb-public-gateway@sha256:[0-9a-f]{64}", gateway.get("reference", ""))):
...
def inspect_release(api, repo, release_id):
    ...
    if RELEASE_ID.fullmatch(release_id) is None:
        raise ValueError("expected an independent usdb-public-vX.Y.Z release tag")
    ...
    git(repo, "merge-base", "--is-ancestor", revision, "refs/remotes/origin/master")
    ...
    run = resolve_build(api.json(
        f"actions/workflows/usdb-public-release.yml/runs?event=push&head_sha={revision}&per_page=100",
    ...
    with tempfile.TemporaryDirectory(prefix="usdb-public-assets-") as temporary:
        ...
        gateway = verify_payload(directory, release_id, revision,
                                 lambda name: git(repo, "show", f"{revision}:public-services/{name}"))
