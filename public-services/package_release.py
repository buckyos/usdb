...
from public_config import DIGEST_IMAGE, image_lock, read_json
from release_installer import render_installer

FILES = ("usdb-public", "usdb_public.py", "public_config.py", "public_checks.py", "config.example.json",
         "assets/images.lock.json", "networks/usdb-testnet-v0.json", "README.md")


def check_network(repo):
    """The public catalog is derived from the same frozen network as node releases."""
    sys.path.insert(0, str(repo / "docker/scripts/tools"))
    from release_manifest import build_network_identity
    actual = build_network_identity(repo / "docker/networks/testnet-v0")
    frozen = read_json(repo / "public-services/networks/usdb-testnet-v0.json")
    if actual != frozen:
        raise ValueError("public network catalog differs from the canonical bundle; regenerate and review it before release")
...
def package(repo, output, version, gateway_image):
    ...
    check_network(repo)
    source = repo / "public-services"
    lock = image_lock(source)
    lock["images"]["gateway"] = {"tag": "usdb-public-v" + version, "reference": gateway_image}
    revision = subprocess.run(["git", "rev-parse", "HEAD"], cwd=repo, check=True, capture_output=True, text=True).stdout.strip()
    dirty = bool(subprocess.run(["git", "status", "--porcelain", "--", "public-services", "tools/public-rpc-gateway"],
                                cwd=repo, check=True, capture_output=True, text=True).stdout.strip())
    output.mkdir(parents=True, exist_ok=True)
    name = "usdb-public-v" + version
    ...
    with tempfile.TemporaryDirectory(prefix="usdb-public-package-") as temporary:
        ...
        for relative in FILES:
            ...
            target.chmod(0o755 if relative == "usdb-public" else 0o644)
        ...
        (root / "release.json").write_text(json.dumps({"schema_version": "usdb-public-release:v1", "version": version,
            "source_revision": revision, "source_dirty": dirty, "config_schema": "usdb-public-config:v1",
