...
验证多 `pass` candidate-set payload 的 `usdb_index_protocol_version` 被篡改后，整条 candidate-set 历史校验路径会稳定落到 `VERSION_MISMATCH`。
...
2. 篡改协议版本后，candidate-set 历史校验统一返回 `VERSION_MISMATCH`，而不是出现不一致的混合错误。
