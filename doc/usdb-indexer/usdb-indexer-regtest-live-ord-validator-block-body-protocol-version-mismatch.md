...
验证 validator block-body payload 的 `usdb_index_protocol_version` 被篡改后，历史 context 校验会稳定落到 `VERSION_MISMATCH`。
...
1. 原始 payload 正常通过。
2. 篡改协议版本后，`state ref / pass snapshot / pass energy` 三条路径都返回 `VERSION_MISMATCH`。
