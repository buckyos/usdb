...
当前 regtest 下，两张 pass 的 `effective_energy` 可能打平。因此脚本不硬编码 `pass1` 一定获胜，而是按 payload 声明的选择规则动态计算 winner：

- `effective_energy DESC`
- 若 `effective_energy` 相同，则按 `inscription_id` 字典序升序 tie-break
...
- `miner_selection` / `candidate_passes` 都携带 `raw_energy / collab_contribution / effective_energy`
...
2. `candidate_passes` 的 winner 满足 `effective_energy DESC + inscription_id ASC`。
