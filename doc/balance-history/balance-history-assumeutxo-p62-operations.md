...
```text
校验配置、BTC 网络、B/G 固定 hash 和 stable lag
  -> bootstrap-staging 中导入 Core UTXO 文件
  -> 安装内置 C(B)/D(B)，按 canonical 区块重放 B+1..G
  -> 计算 G 的规范 UTXO/余额投影
...
- 原生模式本阶段使用 RPC 取块，避免触发现有 LocalLoader 的创世连链/单追加文件假设；LocalLoader 的新路径适配仍为独立 P6.3。
...
2. 在同一个终端设置独立目录与现有供块节点信息。以下沿用本机前述8332端口和cookie位置；执行前确认仍与实际节点一致。
   ...
   供块节点必须提供935001起所有所需 canonical 区块，且 tip 至少963810，以满足已有 stable lag=10。
...
主网完整流程的时间、内存和磁盘峰值仍待本节运行后记录。该验证无需等待 P6.3，但本阶段的追平性能采用 RPC 路径。
