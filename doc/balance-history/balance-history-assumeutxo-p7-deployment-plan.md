...
| P7.1 服务配置和入口 | 本批已实现，入口回归通过，待提交 | 原生配置身份、启动行为、错误配置阻断、旧模式回归 |
| P7.2 Core 镜像与快照编排 | 待实现 | 31.1 供应链校验、下载恢复、loadtxoutset 生命周期、前后台独立状态 |
...
Bitcoin 镜像、现有网络 bundle、默认模式和运行中的节点均未在本批切换。
