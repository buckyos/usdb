...
本文档定义 `stable_lag=5` 下 BTC reorg 深度边界的 deterministic regtest。该矩阵验证
...
```bash
EXPECTED_STABLE_LAG=5 REORG_DEPTH=6 \
...
3. 再挖 `stable_lag=5` 个区块，使 BTC tip 固定为 `H+5`。
...
| --- | --- | --- | --- |
| `lag - 1 = 4` | `H + 2` | 不变 | `125000000 sat` |
| `lag = 5` | `H + 1` | 不变 | `125000000 sat` |
| `lag + 1 = 6` | `H` | 被替换 | `0 sat` |
...
| 变量 | 默认值 | 用途 |
| --- | --- | --- |
| `EXPECTED_STABLE_LAG` | `5` | 当前 regtest registry 固定 lag |
| `REORG_DEPTH` | `4` | 单案例深度，只接受 `lag-1/lag/lag+1` |
