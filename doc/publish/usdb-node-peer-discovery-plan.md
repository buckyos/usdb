...
状态：第一、二阶段代码已实现，本地回归覆盖配置、故障恢复、Compose 渲染与真实回环 P2P；
第二阶段 Docker 29 同机跨网络的 UDP/TCP、模式切换及身份保持验收已通过；公网出站与真实两机验收仍待完成。
第三阶段仍是计划。
IPv6 操作见 [IPv6 与双栈运维](./usdb-node-ipv6-operations.md)。
...
已由第二阶段提供配置与检查，见上述运维文档；同一公钥多个地址之间的自动故障切换尚未实现。
