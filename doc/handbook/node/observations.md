...
本页描述结构化证据和持久事故的只读观测。[核心 monitor](monitor.md)在此基础上增加常驻规则、
事件历史和告警状态。邮件/webhook 通过[通知配置](notifications.md)接入；维护静默和外部心跳不在首期范围。
...
也不能证明数据已经修复。业务事故解除依赖实际修复流程；monitor 取得连续新鲜 READY 与可读空事故源后自动记录恢复，无需 ack 或人工消警。
