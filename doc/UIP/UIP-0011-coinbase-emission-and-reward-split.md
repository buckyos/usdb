...
- 辅助算力池提交格式、BTC 难度证明或多提交者分配规则，见 UIP-0014。
...
| `AUX_POOL_COINBASE_BPS` | `2500` | 等 UIP-0014 激活 | 辅助算力池启用后的 CoinBase 份额。 |
...
| `aux_pool_state` | UIP-0014 | 辅助算力池是否启用及接收方。 |
...
在 UIP-0014 激活且 aux pool 对当前区块有效时，使用设计大纲中的 75% / 25% 目标：
...
在 UIP-0014 Final 前，public network 不应启用 aux pool split。
...
- `aux_pool_reward_atoms` 发给 UIP-0014 定义的 aux pool recipient。
...
| aux pool split 是否从 genesis 启用 | 当前建议 UIP-0014 Final 前不启用。 | UIP-0014 定义证明格式和 recipient 后再激活。 |
