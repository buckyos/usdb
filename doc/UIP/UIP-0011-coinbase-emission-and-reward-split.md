...
- `price` / `real_price` 的更新规则、挂单证明或收敛公式，见 UIP-0012。
- 辅助算力池提交格式、BTC 难度证明或多提交者分配规则，见 UIP-0013。
...
| `price_atoms_per_btc` | 1 BTC 对应的 USDB atoms 数量，由 UIP-0012 price state 提供。 |
| `k_bps` | 协作效率系数 `K` 的 basis points 表示，`10000` 表示 `K = 1.0`。 |
...
如果未来只修改 `K` 函数、手续费比例或 aux pool 分配比例，但 UIP-0007 payload 字节布局不变，应升级上述 policy version，而不是强制升级 `payload_version`。
...
| `AUX_POOL_COINBASE_BPS` | `2500` | 等 UIP-0013 激活 | 辅助算力池启用后的 CoinBase 份额。 |
...
| `price_atoms_per_btc` | UIP-0012 price state | 当前区块使用的 BTC 价格。 |
| `total_miner_btc_sats` | USDB / BTC historical state | 当前发行目标的 BTC 资产总量。 |
| `issued_usdb_atoms` | ETHW state before block | 当前区块执行前已发行 USDB native token 总量。 |
| `k_bps` | 本文 `K` policy | 协作效率系数。 |
| `aux_pool_state` | UIP-0013 | 辅助算力池是否启用及接收方。 |
...
`issued_usdb_atoms` 是 CoinBase 公式的已发行供给输入。
...
- 如果当前 ETHW 执行层没有 protocol-level burn，`protocol_burned_atoms = 0`。
- 普通账户之间转账不改变 `issued_usdb_atoms`。

该口径必须能从 ETHW state 或 consensus accumulator 中确定性取得。不能依赖 indexer 的当前统计。
...
`price_atoms_per_btc` 的来源、初始值和更新时机由 UIP-0012 定义。在 UIP-0012 Final 前，本文不能进入 Final。
...
- 在 `K` 函数、时间窗口和历史样本口径未审计完成前，`reward_rule_version = 1` 可以使用 `k_bps = 10000` 作为最小可实现 fallback。
- 如果首个 public network 必须启用动态 `K`，则必须在本文 Final 前固定 `K` 函数、平均窗口、样本选择和缺省值。
- 动态 `K` 必须只依赖可历史重放的 UIP-0006 profile 和 ETHW reward history，不得依赖当前 leaderboard 或浏览器缓存。

动态 `K` 的候选函数、`CE` / `AE` 精确定义和 reorg 语义列入待审计问题。

# CoinBase Split

## 辅助算力池未启用

在 UIP-0013 未激活或 aux pool 未启用时：
...
在 UIP-0013 激活且 aux pool 对当前区块有效时，使用设计大纲中的 75% / 25% 目标：
...
在 UIP-0013 Final 前，public network 不应启用 aux pool split。
...
- `aux_pool_reward_atoms` 发给 UIP-0013 定义的 aux pool recipient。
...
- uncle reward 是否影响 `issued_usdb_atoms` 和 CoinBase emission accumulator。
...
- `issued_usdb_atoms` 必须随 ETHW state 回滚。
- CoinBase emission accumulator 必须随 ETHW state 回滚。
...
- `price_state_id` 或与 UIP-0012 绑定的 price state reference。
...
- reorg 后 `issued_usdb_atoms`、CoinBase accumulator 和 fee split state 正确回滚。
...
| `issued_usdb_atoms` 是否包含 genesis alloc | 当前倾向包含 genesis alloc、prior CoinBase，扣除 protocol burn。 | 需要与 ETHW genesis / bootstrap 资金策略一起确认。 |
| `price_atoms_per_btc` 来源和 scale | 由 UIP-0012 定义；本文只消费 price state。 | UIP-0012 必须固定初始值、更新顺序和 decimal encoding。 |
| 动态 `K` 是否进入首个 public network | 当前建议 v1 fallback 为 `k_bps = 10000`。 | 如果必须启用动态 `K`，需要固定函数、窗口、样本和 reorg 语义。 |
| aux pool split 是否从 genesis 启用 | 当前建议 UIP-0013 Final 前不启用。 | UIP-0013 定义证明格式和 recipient 后再激活。 |
