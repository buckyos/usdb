...
Activation: USDB-chain network activation matrix; first official networks use fixed price policy from genesis
...
如果价格输入来自 validator 实时查询外部 RPC，区块验证会依赖外部节点状态、archive 可用性、source chain reorg 和 RPC 诚实性。这不适合作为 USDB chain 共识路径。

因此，本文将价格设计为本链可重放的协议状态：

```text
UIP-0011 reward verifier
    reads price_atoms_per_btc from parent USDB chain state
```
...
| `PricePolicyRange` | 在特定 USDB chain 网络和高度范围内生效的一条 price policy 配置。 |
| `price_source_kind` | 价格来源类型，例如固定价格、外部以太坊 DeFi 引用或 USDB 自有 DeFi 挂单证明。 |
| `price_policy_version` | 价格状态转换规则版本。 |
| `price_state` | USDB chain reserved system account storage 中由 `stateRoot` 承诺的当前价格状态。 |
...
```text
PricePolicyRange {
    usdb_chain_id: uint64
    usdb_network_type: enum
    start_block: uint64
...
- 同一 `(usdb_chain_id, usdb_network_type)` 下的 range 必须连续且不重叠。
...
- 历史区块重放必须只依赖当时 active range、parent USDB chain state、区块内携带的可验证数据，以及已经写入本链 state 的 source-chain header / proof registry。
...
| --- | --- | --- |
| `FixedPrice` | active `PricePolicyRange`、parent USDB chain price state。 | 不得查询外部价格源。 |
| `ExternalEthereumDefiReference` | 区块内携带的 source proof，或 parent USDB chain state 中已承诺的 source-chain header / proof registry。 | 不得实时查询 Ethereum RPC、archive RPC 或 current DeFi state。 |
| `UsdbNativeDefiOrderBacked` | parent USDB chain state 中的本链 DeFi contract storage，按区块交易确定性执行。 | 不得通过 RPC 查询 current head；不得绕过本地 state trie / EVM state transition。 |

说明：

- 本链 DeFi 查询不是外部 RPC 依赖。validator 可以在执行区块时读取 parent USDB chain state 中的合约 storage，并通过 deterministic EVM / state transition 得到 child state。
...
- price state storage 必须随 USDB chain state 回滚。
...
```text
(usdb_chain_id, usdb_network_type, usdb_block_height)
    -> PricePolicyRange
```
...
- genesis price slots 初始化为 `100_000 USDB native units / BTC`。
