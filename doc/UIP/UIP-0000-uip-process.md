...
- **网络标识**：具体网络名称或链 ID，例如 `btc-mainnet`、`btc-regtest`、`ethw-mainnet`、`ethw-testnet`、`诸王-mainnet`。
...
| 网络类型 | 含义 |
| --- | --- |
| `mainnet` | 正式生产网络，例如 BTC mainnet、ETHW mainnet、诸王 mainnet。 |
| `testnet` | 公开测试网络。 |
...
| ETHW | mainnet | 诸王-mainnet | ethw_block | TBD | Planned | 诸王 mainnet 执行侧激活。 |
| CrossChain | mainnet | btc-mainnet + 诸王-mainnet | governance | TBD | Planned | 同时依赖 BTC 与 ETHW 的规则。 |
...
1. 诸王 mainnet 的稳定 `network_id` 是否就使用 `诸王-mainnet`，还是需要英文/链 ID 格式。
