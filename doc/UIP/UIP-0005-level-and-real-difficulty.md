...
本文定义如何从 UIP-0004 的 `effective_energy` 派生矿工证 `level`，以及如何用 `level` 折算基础挖矿难度得到 `real_difficulty`。

核心规则：

- `level` 只由 `effective_energy` 和 UIP-0005 参数决定。
- `level` 使用整数阈值表计算，禁止运行时使用浮点数、`log` 或平台相关数学库。
- `real_difficulty` 使用 bps 定点整数规则计算。
- `level`、`difficulty_factor_bps` 和 `real_difficulty` 都是派生值，不可继承、不可写回 raw energy ledger。
...
| `base_difficulty` | ETHW validator / mining policy 输入的基础挖矿难度。 |
| `difficulty_factor_bps` | `level` 对难度产生的折算系数，单位 bps。 |
| `real_difficulty` | 应用 `difficulty_factor_bps` 后的实际难度。 |
...
当前代码尚未实现本 UIP 定义的 `level` 和 `real_difficulty`。

在本 UIP 激活前，已有 leaderboard、RPC 或 validator 样例若只使用 raw `energy`，都不应被视为最终协议行为。实现进入本 UIP 后，candidate set、validator payload 和 mining difficulty integration 必须使用本文定义的派生字段和 rounding 规则。
...
RPC 或 validator payload 可以携带以下派生字段：
...
| `base_difficulty` | decimal string | ETHW validator / mining policy 输入的基础难度。 |
| `real_difficulty` | decimal string | 折算后的实际难度。 |
| `formula_version` | string | 绑定 UIP-0005 参数版本。 |

如果 payload 同时携带 `effective_energy` 与 `level`，validator 必须按本文规则重算 `level`。若重算结果与 payload 中的 `level` 不一致，必须拒绝该 payload 或将其标记为 invalid。

如果 payload 同时携带 `base_difficulty`、`difficulty_factor_bps` 与 `real_difficulty`，validator 必须重算 `real_difficulty`。若重算结果不一致，必须拒绝该 payload 或将其标记为 invalid。

# 历史查询与 Reorg 语义

`level(pass, h)` 和 `real_difficulty(pass, h)` 是高度相关的派生视图：
...
- BTC reorg 改变 pass 状态、raw energy 或 collab contribution 时，`level` 必须随 `effective_energy` 重算。
...
| UIP-0005 | 从 `effective_energy` 派生 `level`，从 `level` 折算 `real_difficulty`。 |
| UIP-0006 | 定义 validator payload 如何携带或验证这些派生字段。 |
...
- `real_difficulty` 使用向上取整，例如 `base_difficulty = 101, factor = 9900` 时结果为 `100`。
- payload 中携带的 `level` / `real_difficulty` 与重算结果不一致时拒绝。
- 参数表变更时，历史高度按当时激活版本重算。

# 待审计问题

1. 是否固定 `MAX_LEVEL = 50` 和最大 50% difficulty discount。
2. `base_difficulty` 的具体来源和数据类型是否应在 UIP-0006 中固定为 ETHW `uint256` decimal string。
3. validator payload 是否必须携带 `level` 和 `difficulty_factor_bps`，还是只携带 `effective_energy` 并由 validator 本地重算。
4. 不同 ETHW 网络是否允许使用不同 level 参数；若允许，必须进入 UIP-0007 activation matrix。
