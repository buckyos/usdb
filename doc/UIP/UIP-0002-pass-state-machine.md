...
类型转换只通过新 mint 完成。直接修改已有 pass 的类型是禁止的。

collab pass 转 standard pass 或 standard pass 转 collab pass 时，状态机只负责保证 `prev` 原子消费。额外折损、cooldown 和有效能量影响由 UIP-0003 / UIP-0004 定义。
...
5. 在 UIP-0003 定义 energy 终态记录格式、继承折损、collab 转 standard 的折损率和 cooldown。
