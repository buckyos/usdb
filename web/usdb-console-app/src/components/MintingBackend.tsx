...
export function MintingBackend({ snapshot }: { snapshot?: MonitorSnapshot }) {
  ...
  return <section className="console-card grid gap-3" aria-label={text('矿工证铸造依赖', 'Miner-pass minting dependencies')}>
    <div className="flex flex-wrap justify-between gap-2"><h2 className="font-semibold">{text('矿工证铸造依赖（可选）', 'Miner-pass minting dependencies (optional)')}</h2><strong role="status" className="status-pill" data-tone={mintingTone(state)}>{mintingLabel(state, locale)}</strong></div>
    ...
    {monitorStatus !== 'available' && <p role="alert">{text(
      '监控未启用或观测已过期，不能据此判断 Ord 离线。在节点执行 usdb-node up --no-watch 检查后台采集服务；前台模式可持续运行 usdb-node console monitor。',
      'Host monitoring is missing or unavailable; this does not establish that Ord is offline. Run usdb-node up --no-watch to check the host observer; foreground mode can keep usdb-node console monitor running.',
    )}</p>}
    {value?.enabled && <>
      ...
      <dl className="grid gap-3 text-sm sm:grid-cols-3">
        {[[text('Bitcoin 最新区块高度', 'Bitcoin latest block height'), value.core_height],
          [text('历史区块验证高度', 'Historical validation height'), value.history_height],
          [text('交易索引高度', 'Txindex height'), value.txindex_height],
          [text('Ord 已索引高度', 'Ord indexed height'), value.ord_height],
          [text('Ord 待同步区块数', 'Ord block gap'), value.ord_gap],
    ...
    {state === 'WAITING_TXINDEX' && <p className="text-sm">{text('Ord 容器正在等待 txindex 覆盖 Bitcoin 最新区块高度，Ord 本体及 HTTP 服务尚未启动；这是正常等待，完成后自动开始 Ord 索引，无需重启。若索引始终不存在，请检查 BTC_TXINDEX=1。', 'The container is waiting for txindex to cover the Bitcoin latest block height; the Ord binary and HTTP server have not started yet. Ord indexing starts automatically when ready; no restart is needed. If txindex is absent, verify BTC_TXINDEX=1.')}</p>}
