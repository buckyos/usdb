...
class PublicServicesTests(unittest.TestCase):
    ...
    def test_cli_lifecycle_from_extracted_release_without_node_installation(self):
        ...
        kit = install / "usdb-public-v0.1.0"
    ...
    def test_release_rejects_mutable_image_and_network_catalog_drift(self):
        with self.assertRaisesRegex(ValueError, "digest"):
            RELEASE.package(ROOT, self.root / "out", "0.1.0", "example:latest")
        with mock.patch.object(RELEASE, "read_json", return_value={}):
            with self.assertRaisesRegex(ValueError, "canonical"):
                RELEASE.check_network(ROOT)
