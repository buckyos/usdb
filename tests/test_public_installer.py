...
sys.path.insert(0, str(ROOT / "public-services"))
...
class PublicInstallerTests(unittest.TestCase):
    ...
    def make_release(self, version):
        image = "ghcr.io/buckyos/usdb-public-gateway@sha256:" + "ab" * 32
    ...
    def run_installer(self, version="0.1.0", *, extra=(), process_substitution=False):
        script = self.assets / ("install-usdb-public-v" + version + ".sh")
        options = ["--install-root", str(self.storage), "--bin-dir", str(self.commands), "--config-file", str(self.config), *extra]
        if process_substitution:
            url = "https://github.com/buckyos/usdb/releases/download/usdb-public-v" + version + "/" + script.name
    ...
    def test_one_line_install_creates_a_working_command_and_private_config(self):
        ...
        self.assertEqual(os.readlink(self.storage / "current"), "releases/usdb-public-v0.1.0")
    ...
    def test_reinstall_and_upgrade_preserve_operator_config_and_deployment_data(self):
        ...
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertEqual(os.readlink(self.storage / "current"), "releases/usdb-public-v0.2.0")
    ...
    def test_corrupt_download_cannot_switch_existing_release(self):
        ...
        self.assertFalse((self.storage / "releases/usdb-public-v0.2.0").exists())

    def test_installed_release_tampering_is_not_silently_overwritten(self):
        self.assertEqual(self.run_installer().returncode, 0)
        target = self.storage / "releases/usdb-public-v0.1.0/usdb_public.py"
    ...
    def test_installer_pins_cannot_be_overridden_by_user_arguments(self):
        result = self.run_installer(extra=("--release-id", "usdb-public-v9.9.9", "--expected-sha256", "0" * 64,
                                           "--archive", str(self.root / "missing")))
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertEqual(os.readlink(self.storage / "current"), "releases/usdb-public-v0.1.0")

    def test_archive_traversal_and_links_are_rejected_without_external_writes(self):
        for index, (name, kind) in enumerate((("usdb-public-v0.1.0/../../outside", tarfile.REGTYPE),
                                             ("usdb-public-v0.1.0/linked", tarfile.SYMTYPE),
                                             ("usdb-public-v0.1.0/hardlinked", tarfile.LNKTYPE),
            ...
            with self.assertRaisesRegex(ValueError, "unsafe"):
                INSTALL.install(archive, "usdb-public-v0.1.0", INSTALL.sha256(archive), self.storage, self.commands, self.config)
    ...
    def test_wrong_manifest_version_cannot_be_installed_even_with_matching_archive_hash(self):
        ...
        with tarfile.open(archive, "w:gz") as tar:
            info = tarfile.TarInfo("usdb-public-v0.1.0/release.json")
            info.size = len(body)
    ...
    def test_generated_script_help_and_asset_checksums(self):
        script = self.assets / "install-usdb-public-v0.1.0.sh"
