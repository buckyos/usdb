...
def make_public_release(root, source, packager):
    ...
    repo = root / "repo"
    (repo / "public-services").mkdir(parents=True)
    for name in (*packager.FILES, "install_release.py"):
        target = repo / "public-services" / name
        target.parent.mkdir(parents=True, exist_ok=True)
        shutil.copyfile(source / "public-services" / name, target)
    commands = (["init", "--initial-branch=master"], ["config", "user.name", "Release fixture"],
                ["config", "user.email", "fixture@example.invalid"], ["add", "."],
                ["-c", "commit.gpgsign=false", "commit", "-m", "Create public release fixture"],
                ["-c", "tag.gpgsign=false", "tag", "-a", "usdb-public-v0.1.0", "-m", "Public fixture"],
    ...
    with patch.object(packager, "check_network"):
        packager.package(repo, assets, "0.1.0", "ghcr.io/buckyos/usdb-public-gateway@sha256:" + "ab" * 32)
    return repo, assets
...
class PublicReleaseAPI:
    ...
    def __init__(self, repo, assets):
        ...
        self.runs = [{"id": 101, "run_attempt": 1, "path": ".github/workflows/usdb-public-release.yml",
    ...
    def json(self, endpoint, *, fields=None, paginate=False):
        ...
        if endpoint.startswith("actions/workflows/usdb-public-release.yml/runs?"):
            assert paginate
    ...
    def public_download(self, url, destination):
        ...
        prefix = f"https://github.com/buckyos/usdb/releases/download/{self.tag}/"
...
def rewrite_public_archive(root, repo, assets, mutate, renderer):
    ...
    installer.write_text(renderer.render_installer(repo / "public-services", archive, release_id))
