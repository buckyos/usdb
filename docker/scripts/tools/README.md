...
| [snapshot_distribution.py](/home/bucky/work/usdb/docker/scripts/tools/snapshot_distribution.py) | immutable snapshot distribution | prepares v2 content-addressed records, performs tuned AWS CLI multipart uploads, and resumes verified parallel HTTPS Range installs | after lightweight exact-height finalize or before a node's first `up` | publisher credentials stay in AWS profile/env; transfer state is resumable; full RocksDB restore is a separate `validate-install` drill |
