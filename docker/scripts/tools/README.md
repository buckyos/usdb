...
| [usdb_node.py](/home/bucky/work/usdb/docker/scripts/tools/usdb_node.py) | Release node controller for host prep, setup, optional firewall, preflight, activation and lifecycle operations | generated private `node.env` | `bitcoin + runtime + frozen network` | locked, bounded and resumable startup | `resume` only continues approved snapshot/start transitions; blocked/degraded states preserve evidence and require operator action |
