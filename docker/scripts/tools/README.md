...
| [snapshot_distribution.py](/home/bucky/work/usdb/docker/scripts/tools/snapshot_distribution.py) | immutable snapshot distribution | prepares v2 content-addressed records, uploads through AWS CLI, and resumes verified HTTPS installs | after lightweight exact-height finalize or before a node's first `up` | publisher credentials stay in AWS profile/env; full RocksDB restore is a separate `validate-install` drill |
