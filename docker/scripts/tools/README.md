...
| [usdb_node.py](/home/bucky/work/usdb/docker/scripts/tools/usdb_node.py) | Release node controller for host prep, setup, optional firewall, preflight, activation and lifecycle operations | generated private `node.env` | `bitcoin + runtime + frozen network` | resumable readiness-ordered startup | `status --json` reports lifecycle `overall_state`, checks and next actions; readiness is probed only after core containers are running |
