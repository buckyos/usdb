...
| [usdb_node.py](/home/bucky/work/usdb/docker/scripts/tools/usdb_node.py) | Release node controller for host prep, setup, optional firewall, preflight, activation and lifecycle operations | generated private `node.env` | `bitcoin + runtime + frozen network` | systemd-supervised, locked, bounded and resumable bootstrap | default `up/resume` submit the controller and only attach progress; activation remains explicit, while blocked/degraded states preserve evidence and require operator action |
