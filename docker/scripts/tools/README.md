...
| [run_testnet_runtime.sh](/home/bucky/work/usdb/docker/scripts/tools/run_testnet_runtime.sh) | Run one testnet-v0 node from versioned images and bundle inputs | uncommitted `docker/networks/testnet-v0/node.env` | `runtime + testnet-v0 network` | phased detached data/runtime start | bind-mounted state below one host data root; no destructive reset action |
...
| [usdb_node.py](/home/bucky/work/usdb/docker/scripts/tools/usdb_node.py) | Release node controller for host prep, setup, optional firewall, preflight, activation and lifecycle operations | generated private `node.env` | `bitcoin + runtime + frozen network` | systemd-supervised, locked, bounded and resumable bootstrap | `setup` installs the controller by default; `up` idempotently submits it and only attaches progress; activation remains explicit, while blocked/degraded states preserve evidence and require operator action |
...
- `testnet-v0` is a resettable development testnet, not a public release candidate.
- future mainnet tools must keep the same explicit bundle/node split instead of
  copying local workspace mounts into deployment profiles.
...
| [release_manifest.py](/home/bucky/work/usdb/docker/scripts/tools/release_manifest.py) | release candidate identity | creates or validates the digest-pinned three-repository manifest | after all three GHCR candidates and component CI checks pass | v7 binds the split v3 snapshot record, required core artifact and optional registry sidecar from the frozen bundle |
| [release_notes.py](/home/bucky/work/usdb/docker/scripts/tools/release_notes.py) | cross-repository release changes | validates append-only change fragments, inventories frozen Git ranges, and derives compatibility impact from consecutive manifests | inside Candidate generation and again before Publish | v2 supports multiple distinct fragment trailers per commit and reports unclassified commits without blocking; JSON and Markdown outputs are checksum-bound release assets |
| [release_candidate_resolver.py](/home/bucky/work/usdb/docker/scripts/tools/release_candidate_resolver.py) | release candidate discovery | validates the Go compatibility lock and resolves the two unique image-producing release-tag builds | inside the candidate manifest workflow before OCI provenance verification | rejects branch builds, another release ID, or missing/ambiguous tag builds; final identity remains digest-pinned |
| [release_publish_resolver.py](/home/bucky/work/usdb/docker/scripts/tools/release_publish_resolver.py) | release promotion validation | resolves one tag-scoped candidate artifact and verifies an existing GitHub Release byte-for-byte | before and after the Environment-protected publish job | rejects expired/ambiguous candidates and any release title, notes, flag, asset-set, or digest drift |
...
| [prepare_usdb_firewall.sh](/home/bucky/work/usdb/docker/scripts/tools/prepare_usdb_firewall.sh) | host firewall setup | validates node.env bind addresses and checks or applies the UFW ingress baseline | after creating node.env and before starting release containers | USDB P2P is public; Bitcoin P2P is private by default and can be explicitly public; upstream firewalls remain an operator responsibility |
| [snapshot_distribution.py](/home/bucky/work/usdb/docker/scripts/tools/snapshot_distribution.py) | immutable snapshot distribution | prepares split v3 content-addressed records, publishes core/registry objects, and independently resumes verified parallel HTTPS Range installs | after lightweight exact-height finalize or before a node's first `up` | core is the startup gate; registry is optional and receives its active pointer only after native signature/integrity verification |
| [install_usdb_node.sh](/home/bucky/work/usdb/docker/scripts/tools/install_usdb_node.sh) | immutable release installer | verifies and installs a published manifest/node-kit pair | first action after host prerequisites | installs no secret or node data; repeated identical install is allowed |
| [prepare_release_installer.py](/home/bucky/work/usdb/docker/scripts/tools/prepare_release_installer.py) | release-bound installer generator | freezes one release ID, asset URL and installer/manifest/node-kit digests | protected GitHub Release workflow | produces the one-command testnet entrypoint; not used at node runtime |
