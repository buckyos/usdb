...
| [usdb_node.py](/home/bucky/work/usdb/docker/scripts/tools/usdb_node.py) | Release node controller for host prep, setup, firewall, preflight, activation and lifecycle operations | generated private `node.env` | `bitcoin + runtime + frozen network` | resumable readiness-ordered startup | `setup` confirms the detected SSH port; `doctor` is read-only host/firewall preflight; `activate-release` only updates image digests for the same bundle |
