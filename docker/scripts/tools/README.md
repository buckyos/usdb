...
| [usdb_node.py](/home/bucky/work/usdb/docker/scripts/tools/usdb_node.py) | Release node controller for host prep, setup, optional firewall, preflight, activation and lifecycle operations | generated private `node.env` | `bitcoin + runtime + frozen network` | resumable readiness-ordered startup | `setup` defaults to external firewall ownership; managed UFW is explicit; `doctor` always checks safe binds; `activate-release` only updates release-owned fields for the same bundle |
