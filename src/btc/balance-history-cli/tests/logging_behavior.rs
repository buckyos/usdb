...
fn help_does_not_initialize_runtime_logging() {
    let output = Command::new(env!("CARGO_BIN_EXE_balance-history-cli"))
        .arg("--help")
        .output()
        .unwrap();
...
fn rpc_failure_uses_stderr_without_polluting_stdout_or_creating_log_files() {
    let home = TestHome::new("balance-history-cli-logging");
    let output = Command::new(env!("CARGO_BIN_EXE_balance-history-cli"))
        .args(["--url", &closed_local_endpoint(), "network-type"])
        .env("HOME", home.path())
    ...
    assert!(String::from_utf8_lossy(&output.stderr).contains("Balance-history CLI command failed"));
