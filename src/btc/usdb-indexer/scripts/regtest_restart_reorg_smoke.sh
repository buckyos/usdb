...
WORK_DIR="${WORK_DIR:-$(mktemp -d /tmp/usdb-indexer-restart-reorg-smoke-XXXXXX)}"
...
WALLET_NAME="${WALLET_NAME:-usdbrestartreorg}"
...
REGTEST_LOG_PREFIX="[usdb-restart-reorg-smoke]"
...
main() {
  ...
  regtest_log "Stopping services before offline height-regression reorg at tip height=${TARGET_HEIGHT}"
  ...
  regtest_wait_until_rpc_expr_eq \
    "balance-history snapshot stable hash after restart rollback" \
  ...
  regtest_wait_until_rpc_expr_eq \
    "usdb snapshot local synced height after restart rollback" \
    ...
    "$((TARGET_HEIGHT - 1))"
  regtest_wait_until_rpc_expr_eq \
    "usdb snapshot upstream stable height after restart rollback" \
    regtest_rpc_call_usdb_indexer \
    "get_snapshot_info" \
    "[]" \
    ...
    "$((TARGET_HEIGHT - 1))"
  regtest_wait_until_rpc_expr_eq \
    "usdb snapshot stable hash after restart rollback" \
    regtest_rpc_call_usdb_indexer \
    "get_snapshot_info" \
    "[]" \
    "((data.get('result') or {}).get('stable_block_hash', ''))" \
  ...
  regtest_assert_usdb_db_scalar \
    ...
    "0" \
    "future pass_block_commits cleared after restart rollback"
  regtest_assert_usdb_db_scalar \
    "SELECT COUNT(*) FROM active_balance_snapshots WHERE block_height > $((TARGET_HEIGHT - 1));" \
    "0" \
    "future active_balance_snapshots cleared after restart rollback"
  regtest_assert_usdb_db_scalar \
    "SELECT COUNT(*) FROM state WHERE name = 'upstream_reorg_recovery_pending_height';" \
    "0" \
    "pending recovery marker cleared after restart rollback"
  ...
  regtest_wait_until_rpc_expr_eq \
    ...
    "$new_bh_commit"
  regtest_wait_until_rpc_expr_eq \
    "usdb pass block commit anchor after restart replay" \
    regtest_rpc_call_usdb_indexer \
  ...
  regtest_log "USDB indexer restart height-regression reorg smoke test succeeded."
