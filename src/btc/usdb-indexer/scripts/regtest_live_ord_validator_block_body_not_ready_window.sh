...
WORK_DIR="${WORK_DIR:-$(mktemp -d /tmp/usdb-indexer-live-ord-validator-block-body-not-ready-window-XXXXXX)}"
...
BTC_RPC_PORT="${BTC_RPC_PORT:-31232}"
BTC_P2P_PORT="${BTC_P2P_PORT:-31233}"
BH_RPC_PORT="${BH_RPC_PORT:-31210}"
USDB_INDEXER_RPC_PORT="${USDB_INDEXER_RPC_PORT:-31220}"
ORD_RPC_PORT="${ORD_RPC_PORT:-31230}"
WALLET_NAME="${WALLET_NAME:-usdbvalidatornotreadywindow}"
ORD_WALLET_NAME="${ORD_WALLET_NAME:-ord-validator-not-ready-window-a}"
ORD_WALLET_NAME_B="${ORD_WALLET_NAME_B:-ord-validator-not-ready-window-b}"
...
LAG_BLOCKS_WHILE_DOWN="${LAG_BLOCKS_WHILE_DOWN:-3}"
...
REGTEST_LOG_PREFIX="[usdb-validator-block-body-not-ready-window]"
...
main() {
  ...
  local current_tip_height historical_height state_ref_resp pass_snapshot_resp pass_energy_resp
  local payload_file continue_address
  ...
  mint_content_file="$WORK_DIR/usdb_validator_block_body_not_ready_window_mint.json"
  cat >"$mint_content_file" <<'EOF'
{"p":"usdb","op":"mint","v":1,"usdb_main":"0x1919191919191919191919191919191919191919","prev":[]}
EOF
  ...
  payload_file="$WORK_DIR/validator_block_body_not_ready_window_payload.json"
  ...
  regtest_log "Creating a catch-up window where rpc_alive=true but consensus_ready=false"
  regtest_stop_usdb_indexer
  regtest_stop_balance_history
  continue_address="$(regtest_get_new_address)"
  regtest_mine_blocks "$LAG_BLOCKS_WHILE_DOWN" "$continue_address"
  regtest_wait_until_ord_server_synced_to_bitcoind
  ...
  regtest_validate_validator_payload_versioned_consensus_error "$payload_file" "-32041" "SNAPSHOT_NOT_READY"
  ...
  regtest_validate_validator_payload_versioned_success "$payload_file"
  regtest_log "USDB validator block-body not-ready window test succeeded."
}
