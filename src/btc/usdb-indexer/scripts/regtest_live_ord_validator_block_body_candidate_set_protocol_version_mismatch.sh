...
WORK_DIR="${WORK_DIR:-$(mktemp -d /tmp/usdb-indexer-live-ord-validator-block-body-candidate-set-protocol-version-mismatch-XXXXXX)}"
...
ORD_WALLET_NAME_B="${ORD_WALLET_NAME_B:-ord-validator-candidate-set-protocol-b}"
...
REGTEST_LOG_PREFIX="[usdb-validator-block-body-candidate-set-protocol-version-mismatch]"
...
main() {
  ...
  for idx in 0 1 2; do
    ...
    mint_file="$WORK_DIR/usdb_validator_candidate_set_protocol_mint_$((idx + 1)).json"
    cat >"$mint_file" <<EOF
{"p":"usdb","op":"mint","v":1,"usdb_main":"0x$(printf '%040x' $((idx + 41)))","prev":[]}
EOF
  ...
  valid_payload="$WORK_DIR/validator_block_body_candidate_set_protocol_version_valid_payload.json"
  ...
  tampered_payload="$WORK_DIR/validator_block_body_candidate_set_protocol_version_tampered_payload.json"
