...
WORK_DIR="${WORK_DIR:-$(mktemp -d /tmp/usdb-indexer-live-ord-validator-block-body-api-version-mismatch-XXXXXX)}"
...
USDB_INDEXER_RPC_PORT="${USDB_INDEXER_RPC_PORT:-30620}"
ORD_RPC_PORT="${ORD_RPC_PORT:-30630}"
WALLET_NAME="${WALLET_NAME:-usdbvalidatorapiversionmismatch}"
ORD_WALLET_NAME="${ORD_WALLET_NAME:-ord-validator-api-version-mismatch-a}"
ORD_WALLET_NAME_B="${ORD_WALLET_NAME_B:-ord-validator-api-version-mismatch-b}"
...
REGTEST_LOG_PREFIX="[usdb-validator-block-body-api-version-mismatch]"
...
main() {
  ...
  local current_tip_height historical_height
  local state_ref_resp pass_snapshot_resp pass_energy_resp payload_file tampered_payload_file
  ...
  mint_content_file="$WORK_DIR/usdb_validator_block_body_api_version_mismatch_mint.json"
...
EOF
  ...
  payload_file="$WORK_DIR/validator_block_body_api_version_valid_payload.json"
  ...
  tampered_payload_file="$WORK_DIR/validator_block_body_api_version_tampered_payload.json"
  ...
  regtest_log "USDB validator block-body API-version mismatch test succeeded."
}
