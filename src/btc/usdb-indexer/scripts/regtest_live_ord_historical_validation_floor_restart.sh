...
WORK_DIR="${WORK_DIR:-$(mktemp -d /tmp/usdb-indexer-live-ord-historical-validation-floor-XXXXXX)}"
...
USDB_INDEXER_RPC_PORT="${USDB_INDEXER_RPC_PORT:-29850}"
ORD_RPC_PORT="${ORD_RPC_PORT:-29860}"
WALLET_NAME="${WALLET_NAME:-usdbhistfloor}"
ORD_WALLET_NAME="${ORD_WALLET_NAME:-ord-hist-floor-a}"
ORD_WALLET_NAME_B="${ORD_WALLET_NAME_B:-ord-hist-floor-b}"
...
REGTEST_LOG_PREFIX="[usdb-historical-validation-floor]"
...
assert_historical_context_state_not_retained() {
  local pass_id="$1"
  local block_height="$2"
  local context_json="$3"
  ...
  resp="$(regtest_rpc_call_usdb_indexer "get_state_ref_at_height" "$state_ref_params")"
  regtest_assert_usdb_consensus_error "$resp" "-32048" "STATE_NOT_RETAINED"
...
main() {
  ...
  local system_state_id context_json continue_address retention_floor
