...
WORK_DIR="${WORK_DIR:-$(mktemp -d /tmp/usdb-indexer-live-ord-validator-block-body-payload-version-upgrade-XXXXXX)}"
...
BTC_RPC_PORT="${BTC_RPC_PORT:-30832}"
BTC_P2P_PORT="${BTC_P2P_PORT:-30833}"
BH_RPC_PORT="${BH_RPC_PORT:-30810}"
USDB_INDEXER_RPC_PORT="${USDB_INDEXER_RPC_PORT:-30820}"
ORD_RPC_PORT="${ORD_RPC_PORT:-30830}"
WALLET_NAME="${WALLET_NAME:-usdbvalidatorpayloadupgrade}"
ORD_WALLET_NAME="${ORD_WALLET_NAME:-ord-validator-payload-upgrade-a}"
ORD_WALLET_NAME_B="${ORD_WALLET_NAME_B:-ord-validator-payload-upgrade-b}"
...
REGTEST_LOG_PREFIX="[usdb-validator-payload-version-upgrade]"
...
main() {
  ...
  local payload_v1 payload_v11 candidate_entries_json winner_id continue_address
  ...
  mint_a_file="$WORK_DIR/usdb_validator_payload_version_upgrade_mint_a.json"
  mint_b_file="$WORK_DIR/usdb_validator_payload_version_upgrade_mint_b.json"
  cat >"$mint_a_file" <<'EOF'
...
EOF
  ...
  regtest_assert_json_expr \
    "{\"result\":{\"payload_version\":\"$(regtest_validator_payload_version "$payload_v1")\"}}" \
    "(data.get('result') or {}).get('payload_version')" \
    "1.0.0"
  ...
  regtest_assert_json_expr \
    "{\"result\":{\"payload_version\":\"$(regtest_validator_payload_version "$payload_v11")\"}}" \
    "(data.get('result') or {}).get('payload_version')" \
    "1.1.0"

  regtest_log "Validator must accept both v1.0 single-pass and v1.1 candidate-set payloads during the same upgrade window"
  ...
  regtest_log "Both payload generations must remain replayable after head advances again"
  regtest_validate_validator_payload_versioned_success "$payload_v1"
  regtest_validate_validator_payload_versioned_success "$payload_v11"

  regtest_log "USDB validator block-body payload-version upgrade test succeeded."
