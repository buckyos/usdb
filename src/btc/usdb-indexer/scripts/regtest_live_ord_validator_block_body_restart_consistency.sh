...
WORK_DIR="${WORK_DIR:-$(mktemp -d /tmp/usdb-indexer-live-ord-validator-block-body-restart-consistency-XXXXXX)}"
...
BTC_RPC_PORT="${BTC_RPC_PORT:-31132}"
BTC_P2P_PORT="${BTC_P2P_PORT:-31133}"
BH_RPC_PORT="${BH_RPC_PORT:-31110}"
USDB_INDEXER_RPC_PORT="${USDB_INDEXER_RPC_PORT:-31120}"
ORD_RPC_PORT="${ORD_RPC_PORT:-31130}"
WALLET_NAME="${WALLET_NAME:-usdbvalidatorrestartconsistency}"
ORD_WALLET_NAME="${ORD_WALLET_NAME:-ord-validator-restart-consistency-a}"
ORD_WALLET_NAME_B="${ORD_WALLET_NAME_B:-ord-validator-restart-consistency-b}"
...
RESTART_GAP_BLOCKS="${RESTART_GAP_BLOCKS:-2}"
...
REGTEST_LOG_PREFIX="[usdb-validator-block-body-restart-consistency]"
...
main() {
  ...
  mint_content_file="$WORK_DIR/usdb_validator_block_body_restart_consistency_mint.json"
  cat >"$mint_content_file" <<'EOF'
{"p":"usdb","op":"mint","v":1,"usdb_main":"0x1818181818181818181818181818181818181818","prev":[]}
EOF
  ...
  payload_file="$WORK_DIR/validator_block_body_restart_consistency_payload.json"
  ...
  regtest_log "Gracefully restarting services after payload capture"
  regtest_stop_usdb_indexer
  regtest_stop_balance_history
  continue_address="$(regtest_get_new_address)"
  regtest_mine_blocks "$RESTART_GAP_BLOCKS" "$continue_address"
  regtest_wait_until_ord_server_synced_to_bitcoind
  ...
  regtest_validate_validator_payload_versioned_success "$payload_file"
  regtest_log "USDB validator block-body restart consistency test succeeded."
}
