...
main() {
  ...
  local target_tip_height rollback_height

  if [[ ! "$BTC_STABLE_LAG_BLOCKS" =~ ^[0-9]+$ ]] || (( BTC_STABLE_LAG_BLOCKS <= 0 )); then
    regtest_log "BTC_STABLE_LAG_BLOCKS must be a positive integer"
    exit 1
  fi
  target_tip_height=$((TARGET_HEIGHT + BTC_STABLE_LAG_BLOCKS))
  rollback_height=$((TARGET_HEIGHT - BTC_STABLE_LAG_BLOCKS - 1))
  if (( rollback_height < 0 )); then
    regtest_log "TARGET_HEIGHT must exceed BTC_STABLE_LAG_BLOCKS"
    exit 1
  fi
  ...
  regtest_assert_json_expr "$usdb_snapshot_resp" "((data.get('result') or {}).get('consensus_identity') or {}).get('stable_lag')" "$BTC_STABLE_LAG_BLOCKS"
