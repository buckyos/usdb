...
WORK_DIR="${WORK_DIR:-$(mktemp -d /tmp/usdb-indexer-pending-recovery-transfer-restart-XXXXXX)}"
...
WALLET_NAME="${WALLET_NAME:-usdbpendingtransfer}"
...
REGTEST_LOG_PREFIX="[usdb-pending-recovery-transfer-restart]"
USDB_INDEXER_INJECT_REORG_RECOVERY_TRANSFER_RELOAD_FAILURES="${USDB_INDEXER_INJECT_REORG_RECOVERY_TRANSFER_RELOAD_FAILURES:-1}"
...
main() {
  ...
  regtest_log "Triggering height-regression reorg with injected transfer reload failure at tip height=${TARGET_HEIGHT}"
  ...
  regtest_wait_until_file_contains \
    ...
    "Injected reorg recovery transfer reload failure"
  regtest_wait_until_usdb_db_scalar_eq \
    "SELECT value FROM state WHERE name = 'upstream_reorg_recovery_pending_height';" \
    "$rollback_height" \
    "pending recovery marker present after injected transfer reload failure"
  regtest_wait_until_usdb_synced_eq "$rollback_height"

  regtest_log "Stopping usdb-indexer before automatic retry to preserve pending marker across restart"
  regtest_stop_usdb_indexer
  regtest_assert_usdb_db_scalar \
    "SELECT value FROM state WHERE name = 'upstream_reorg_recovery_pending_height';" \
    "$rollback_height" \
    "pending recovery marker still present after process stop"

  USDB_INDEXER_INJECT_REORG_RECOVERY_TRANSFER_RELOAD_FAILURES=""
  regtest_restart_usdb_indexer
  regtest_wait_until_usdb_synced_eq "$rollback_height"
  regtest_wait_until_rpc_expr_eq \
    "usdb snapshot stable hash after restart recovery" \
  ...
  regtest_wait_until_usdb_db_scalar_eq \
    "SELECT value FROM state WHERE name = 'upstream_reorg_recovery_pending_height';" \
    "" \
    "pending recovery marker cleared after restart resume"

  regtest_assert_usdb_db_scalar \
    "SELECT COUNT(*) FROM pass_block_commits WHERE block_height > ${rollback_height};" \
    "0" \
    "future pass_block_commits cleared after restart resume"
  regtest_assert_usdb_db_scalar \
    "SELECT COUNT(*) FROM active_balance_snapshots WHERE block_height > ${rollback_height};" \
    "0" \
    "future active_balance_snapshots cleared after restart resume"
  ...
  regtest_wait_until_usdb_synced_eq "$TARGET_HEIGHT"
  regtest_wait_until_rpc_expr_eq \
    "usdb snapshot stable hash after restart replay" \
    regtest_rpc_call_usdb_indexer \
    "get_snapshot_info" \
    "[]" \
    "((data.get('result') or {}).get('stable_block_hash', ''))" \
    "$replacement_hash"
  regtest_wait_until_rpc_expr_eq \
    "usdb snapshot latest block commit after restart replay" \
    regtest_rpc_call_usdb_indexer \
    "get_snapshot_info" \
    "[]" \
  ...
  regtest_log "USDB indexer pending recovery transfer reload restart test succeeded."
