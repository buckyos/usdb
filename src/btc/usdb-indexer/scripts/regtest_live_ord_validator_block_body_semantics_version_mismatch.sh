...
WORK_DIR="${WORK_DIR:-$(mktemp -d /tmp/usdb-indexer-live-ord-validator-block-body-semantics-version-mismatch-XXXXXX)}"
...
USDB_INDEXER_RPC_PORT="${USDB_INDEXER_RPC_PORT:-30320}"
ORD_RPC_PORT="${ORD_RPC_PORT:-30330}"
WALLET_NAME="${WALLET_NAME:-usdbvalidatorsemanticsmismatch}"
ORD_WALLET_NAME="${ORD_WALLET_NAME:-ord-validator-semantics-mismatch-a}"
ORD_WALLET_NAME_B="${ORD_WALLET_NAME_B:-ord-validator-semantics-mismatch-b}"
...
REGTEST_LOG_PREFIX="[usdb-validator-block-body-semantics-version-mismatch]"
...
EOF
  ...
  payload_file="$WORK_DIR/validator_block_body_semantics_version_valid_payload.json"
  ...
  tampered_payload_file="$WORK_DIR/validator_block_body_semantics_version_tampered_payload.json"
  ...
  regtest_log "USDB validator block-body semantics-version mismatch test succeeded."
}
