...
WORK_DIR="${WORK_DIR:-$(mktemp -d /tmp/usdb-indexer-live-ord-validator-block-body-candidate-set-semantics-version-mismatch-XXXXXX)}"
...
USDB_INDEXER_RPC_PORT="${USDB_INDEXER_RPC_PORT:-30520}"
ORD_RPC_PORT="${ORD_RPC_PORT:-30530}"
WALLET_NAME="${WALLET_NAME:-usdbvalidatorcandidatesetsemanticsmismatch}"
ORD_WALLET_NAME="${ORD_WALLET_NAME:-ord-validator-candidate-set-semantics-a}"
ORD_WALLET_NAME_B="${ORD_WALLET_NAME_B:-ord-validator-candidate-set-semantics-b}"
...
REGTEST_LOG_PREFIX="[usdb-validator-block-body-candidate-set-semantics-version-mismatch]"
...
main() {
  ...
  local candidate_entries_json winner_id valid_payload tampered_payload
  ...
  wallet_names=("$ORD_WALLET_NAME" "$ORD_WALLET_NAME_B" "$ORD_WALLET_NAME")
  candidate_ids=()

  for idx in 0 1 2; do
    wallet_name="${wallet_names[$idx]}"
    receive_address="$(regtest_get_ord_wallet_receive_address "$wallet_name")"
    mint_file="$WORK_DIR/usdb_validator_candidate_set_semantics_mint_$((idx + 1)).json"
    cat >"$mint_file" <<EOF
{"p":"usdb","op":"mint","v":1,"usdb_main":"0x$(printf '%040x' $((idx + 51)))","prev":[]}
EOF
    ...
    regtest_wait_until_ord_server_synced_to_bitcoind
    if (( idx < 2 && AGE_BLOCKS_BETWEEN_PASSES > 0 )); then
  ...
  valid_payload="$WORK_DIR/validator_block_body_candidate_set_semantics_version_valid_payload.json"
  ...
  tampered_payload="$WORK_DIR/validator_block_body_candidate_set_semantics_version_tampered_payload.json"
  regtest_write_validator_payload_tampered_external_state_field \
    "$valid_payload" \
    "$tampered_payload" \
    "balance_history_semantics_version" \
    "balance-snapshot-at-or-before:v999"
  regtest_validate_validator_candidate_set_payload_consensus_error "$tampered_payload" "-32044" "VERSION_MISMATCH"

  regtest_log "USDB validator candidate-set semantics-version mismatch test succeeded."
