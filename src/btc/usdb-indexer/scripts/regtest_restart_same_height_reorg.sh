...
WORK_DIR="${WORK_DIR:-$(mktemp -d /tmp/usdb-indexer-restart-same-height-reorg-XXXXXX)}"
...
WALLET_NAME="${WALLET_NAME:-usdbrestartsameheight}"
...
REGTEST_LOG_PREFIX="[usdb-restart-same-height-reorg]"
...
main() {
  ...
  regtest_log "Stopping services before offline same-height replacement at tip height=${TARGET_HEIGHT}"
  ...
  regtest_wait_until_rpc_expr_eq \
    "usdb snapshot stable hash after restart same-height reorg" \
    ...
    "$replacement_hash"
  regtest_wait_until_rpc_expr_eq \
    "usdb snapshot latest block commit after restart same-height reorg" \
    regtest_rpc_call_usdb_indexer \
    "get_snapshot_info" \
    "[]" \
    ...
    "$new_bh_commit"
  regtest_wait_until_rpc_expr_eq \
    "usdb pass block commit anchor after restart same-height reorg" \
    regtest_rpc_call_usdb_indexer \
  ...
  if [[ "$new_snapshot_id" == "$old_snapshot_id" ]]; then
    regtest_log "Snapshot id did not change after restart same-height reorg: snapshot_id=${new_snapshot_id}"
    exit 1
  fi
  if [[ "$new_snapshot_commit" == "$old_snapshot_commit" ]]; then
    regtest_log "Snapshot latest_block_commit did not change after restart same-height reorg: latest_block_commit=${new_snapshot_commit}"
    exit 1
  fi
  if [[ "$new_pass_anchor" == "$old_pass_anchor" ]]; then
    regtest_log "Pass block commit anchor did not change after restart same-height reorg: balance_history_block_commit=${new_pass_anchor}"
    exit 1
  fi
  ...
  regtest_assert_usdb_db_scalar \
    ...
    "1" \
    "exactly one pass_block_commit row at restart replacement height"
  regtest_assert_usdb_db_scalar \
    "SELECT COUNT(*) FROM active_balance_snapshots WHERE block_height = ${TARGET_HEIGHT};" \
    "1" \
    "exactly one active_balance_snapshot row at restart replacement height"
  regtest_assert_usdb_db_scalar \
    "SELECT COUNT(*) FROM state WHERE name = 'upstream_reorg_recovery_pending_height';" \
    "0" \
    "pending recovery marker cleared after restart same-height reorg"
  ...
  regtest_log "USDB indexer restart same-height reorg smoke test succeeded."
