...
mod tests {
    ...
    impl MockBalanceProvider {
        ...
        fn with_range(
            ...
            self.ranges.lock().unwrap().insert(
                (address, block_range.start, block_range.end),
                items,
            );
            self
        }
    ...
    fn setup_manager_with_mock(
        ...
        let manager = MinerPassManager::new(config, pass_storage.clone(), energy_manager.clone())
            .unwrap();
    ...
    async fn test_scenario_transfer_then_remint_prev_inherits_and_consumes() {
        ...
        let (root_dir, storage, energy_manager, manager) =
            setup_manager_with_mock("scenario_transfer_remint", mock_provider);
        ...
        let old_pass = storage.get_pass_by_inscription_id(&pass_old).unwrap().unwrap();
        assert_eq!(old_pass.owner, owner_b);
        assert_eq!(old_pass.state, MinerPassState::Consumed);

        let new_pass = storage.get_pass_by_inscription_id(&pass_new).unwrap().unwrap();
    ...
    async fn test_scenario_same_owner_remint_prev_consumed_and_single_active() {
        ...
        let (root_dir, storage, energy_manager, manager) =
            setup_manager_with_mock("scenario_same_owner_remint", mock_provider);
