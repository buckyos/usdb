...
impl PassEnergyStorage {
    ...
    pub fn get_pass_energy_records_by_page_in_height_range_desc(
        &self,
        inscription_id: &InscriptionId,
        from_block_height: u32,
        to_block_height: u32,
        ...
        let offset = page.checked_mul(page_size).ok_or_else(|| {
            let msg = format!(
                "Pagination overflow when querying pass energy range desc: page={}, page_size={}",
                page, page_size
            );
            error!("{}", msg);
            msg
        ...
        while let Some(item) = iter.next() {
            ...
            // Records are ordered by (inscription_id, block_height). Once inscription changes
            // while iterating backward, all remaining rows are out of this query scope.
            if key.inscription_id != *inscription_id {
                break;
            }
...
mod tests {
    ...
    #[test]
    fn test_get_pass_energy_records_by_page_in_height_range_desc() {
        let dir = test_data_dir("range_page_desc");
        let storage = PassEnergyStorage::new(&dir).unwrap();

        let target = inscription_id(52, 0);
        let other = inscription_id(53, 1);
        for height in [100, 110, 120, 130] {
            storage
                .insert_pass_energy_record(&PassEnergyRecord {
                    inscription_id: target.clone(),
        ...
        let page0 = storage
            .get_pass_energy_records_by_page_in_height_range_desc(&target, 100, 130, 0, 2)
            .unwrap();
        assert_eq!(page0.len(), 2);
        assert_eq!(page0[0].block_height, 130);
        assert_eq!(page0[1].block_height, 120);

        let page1 = storage
            .get_pass_energy_records_by_page_in_height_range_desc(&target, 100, 130, 1, 2)
            .unwrap();
        assert_eq!(page1.len(), 2);
        assert_eq!(page1[0].block_height, 110);
        assert_eq!(page1[1].block_height, 100);

        let page2 = storage
            .get_pass_energy_records_by_page_in_height_range_desc(&target, 100, 130, 2, 2)
            .unwrap();
        assert!(page2.is_empty());

        let err = storage
            .get_pass_energy_records_by_page_in_height_range_desc(&target, 130, 100, 0, 10)
            .unwrap_err();
        assert!(err.contains("Invalid energy height range"));
