use bitcoincore_rpc::bitcoin::{Amount, Block, BlockHash, OutPoint, ScriptBuf, Transaction, Txid};
...
impl BTCRpcClient {
    ...
    fn update_client(&self) -> Result<(), String> {
        let new_client =
            Client::new(&self.config.rpc_url, self.config.auth.clone()).map_err(|e| {
    ...
    fn is_auth_cookie(&self) -> bool {
        matches!(self.config.auth, Auth::CookieFile(_))
    }
    ...
    pub fn get_transcation(&self, txid: &Txid) -> Result<Transaction, String> {
        self.client()?.get_raw_transaction(txid, None).map_err(|e| {
            ...
            let msg = format!("Failed to get raw transaction {}: {}", txid, e);
            error!("{}", msg);
            msg
        })
    }

    pub fn get_transactions(&self, txids: &[Txid]) -> Result<Vec<Transaction>, String> {
        use rayon::prelude::*;
