...
export REGTEST_LOG_PREFIX="[snapshot-install-corrupt]"
...
main() {
  ...
  local snapshot_manifest corrupt_manifest corrupt_output resp
...
PY
  ...
  corrupt_manifest="$(regtest_snapshot_manifest_path "$corrupt_snapshot_file")"
  regtest_write_snapshot_manifest_variant \
    "$snapshot_manifest" "$corrupt_manifest" "$corrupt_snapshot_file" "$corrupt_snapshot_hash"
