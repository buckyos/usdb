...
regtest_assert_no_install_artifacts() {
  ...
  if [[ "$staging_count" != "0" || "$backup_count" != "0" ]]; then
    regtest_log "Unexpected snapshot install artifacts under ${root_dir}"
...
main() {
  ...
  wrong_hash_output="$WORK_DIR/install_wrong_hash.out"
  missing_file_output="$WORK_DIR/install_missing_file.out"
  ...
  regtest_log "Snapshot install failure test succeeded."
