...
WORK_DIR="${WORK_DIR:-$(mktemp -d /tmp/usdb-bh-restart-same-block-aggregate-reorg-XXXXXX)}"
BITCOIN_DIR="${BITCOIN_DIR:-$WORK_DIR/bitcoin}"
BITCOIN_BIN_DIR="${BITCOIN_BIN_DIR:-/home/bucky/btc/bitcoin-28.1/bin}"
BALANCE_HISTORY_ROOT="${BALANCE_HISTORY_ROOT:-$WORK_DIR/balance-history}"
BTC_RPC_PORT="${BTC_RPC_PORT:-30652}"
BTC_P2P_PORT="${BTC_P2P_PORT:-30653}"
BH_RPC_PORT="${BH_RPC_PORT:-30630}"
WALLET_NAME="${WALLET_NAME:-bhrestartsameblockaggregate}"
SYNC_TIMEOUT_SEC="${SYNC_TIMEOUT_SEC:-120}"
BALANCE_HISTORY_LOG_FILE="${BALANCE_HISTORY_LOG_FILE:-$WORK_DIR/balance-history.log}"
REGTEST_LOG_PREFIX="[restart-same-block-aggregate-reorg]"
...
main() {
  ...
  local funded_height aggregate_height fund_block_hash original_tip_hash replacement_tip_hash resp
  ...
  fund_block_hash="$($BITCOIN_CLI_BIN -regtest -datadir="$BITCOIN_DIR" -rpcport="$BTC_RPC_PORT" getblockhash "$funded_height")"
  ...
  regtest_log "Created same-block bonus payment txid=${bonus_txid}"
  regtest_mine_blocks 1 "$mining_address"
  ...
  regtest_log "Stopping balance-history before offline aggregate-block reorg: funded_height=${funded_height}, aggregate_height=${aggregate_height}"
  ...
  if [[ "$replacement_tip_hash" == "$original_tip_hash" ]]; then
    regtest_log "Replacement tip hash unexpectedly matches original aggregate tip hash"
    exit 1
  fi
  if [[ "$replacement_tip_hash" == "$fund_block_hash" ]]; then
    regtest_log "Replacement tip hash unexpectedly rewound below funded height"
    exit 1
  fi
  ...
  resp="$(regtest_rpc_call_balance_history "get_address_balance" "[{\"script_hash\":\"${script_hash_c}\",\"block_height\":null,\"block_range\":{\"start\":${funded_height},\"end\":$((aggregate_height + 1))}}]")"
  ...
  if [[ "$(regtest_get_snapshot_stable_hash)" != "$replacement_tip_hash" ]]; then
    regtest_log "Snapshot stable hash did not converge to replacement tip hash"
    exit 1
  fi

  regtest_log "Restart same-block aggregate reorg test succeeded."
