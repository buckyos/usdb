...
use bitcoincore_rpc::bitcoin::{ScriptHash, Transaction, OutPoint};
use crate::btc::{BTCClient, BTCClientRef};
use crate::db::{BalanceHistoryDBRef, BalanceHistoryEntry};
use crate::config::{BalanceHistoryConfigRef};
use crate::utxo::{UTXOCacheRef, CacheTxOut, UTXOCache};
use std::sync::Arc;
...
impl BalanceHistoryIndexer {
    pub fn new(config: BalanceHistoryConfigRef, db: BalanceHistoryDBRef) -> Result<Self, String> {
        ...
        Ok(Self { config, btc_client, utxo_cache, db })
    }
    ...
    pub fn run(&self) -> ! {
        ...
        loop {
            match self.sync_once() {
                Ok(latest_height) => {
                    info!("Sync iteration completed successfully. Latest synced height: {}", latest_height);

                    // Wait for new blocks
                    match self.wait_for_new_blocks(latest_height) {
                        Ok(new_height) => {
                            info!("New block detected at height {}. Continuing sync...", new_height);
                        }
                        Err(e) => {
                            error!("Error while waiting for new blocks: {}. Retrying in 10 seconds...", e);
    ...
    fn sync_once(&self) -> Result<u64, String> {
        ...
        if latest_btc_height <= last_synced_height as u64 {
            info!("No new blocks to sync. Latest BTC height: {}, Last synced height: {}", latest_btc_height, last_synced_height);
        ...
        while current_height <= latest_btc_height {
            let end_height = std::cmp::min(current_height + batch_size as u64 - 1, latest_btc_height);
            info!("Processing blocks [{} - {}]", current_height, end_height);
            self.process_block_batch(current_height..=end_height).await?;
    ...
    fn process_block_batch(&self, height_range: std::ops::RangeInclusive<u64>) -> Result<(), String> {
        ...
        info!("Finished processing blocks [{} - {}]", height_range.start(), height_range.end());
        Ok(())
    }
        ...
        for tx in block.txdata.iter() {
            if !tx.is_coinbase() {
                for vin in tx.input.iter() {
                    assert!(!vin.previous_output.is_null(), "Previous output should not be null {}", tx.compute_txid());
                    ...
                    match history.entry(utxo.script_hash) {
                        std::collections::hash_map::Entry::Vacant(e) => {
                            ...
                            if latest_entry.block_height == block_height as u32 {
                                ...
                                warn!("Skipping duplicate entry for script_hash {} at block height {}", utxo.script_hash, block_height);
                                continue;
                            }

                            assert!(latest_entry.block_height < block_height as u32, "Latest entry block height should be less than current block height");
                            assert!(latest_entry.balance >= utxo.value, "Insufficient balance for script_hash {}", utxo.script_hash);
                        ...
                        std::collections::hash_map::Entry::Occupied(mut e) => {
                            ...
                            assert!(entry.balance >= utxo.value, "Insufficient balance for script_hash {}", utxo.script_hash);
            ...
            for (n ,vout) in tx.output.iter().enumerate() {
                ...
                match history.entry(script_hash) {
                    std::collections::hash_map::Entry::Vacant(e) => {
                        ...
                        if latest_entry.block_height == block_height as u32 {
                            // The block may have been synced, skip duplicate entry
                            warn!("Skipping duplicate entry for script_hash {} at block height {}", script_hash, block_height);
                            continue;
                        }
